...
import requests # Asegúrate de que esta línea esté para el envío de correos
...
#########
# =========================================================================
# 🚀 INTERCEPTOR EXCLUSIVO: KPIs + SISTEMA DE ALERTAS DE RETRASO DETALLADO
# =========================================================================
if choice == "Panel de Control":
    st.header("📊 Resumen General y Métricas del Laboratorio")
    
    # 1. Extracción de datos limpia en la Base de Datos
    with obtener_conexion() as conn:
        ...
        df_rank_equipos = pd.read_sql_query("""
            ...
            FROM prestamos GROUP BY id_equipo ORDER BY COUNT(*) DESC LIMIT 5
        """, conn)
        
        df_rank_usuarios = pd.read_sql_query("""
            SELECT usuario as 'Nombre Custodio', rut as 'RUT', COUNT(*) as 'Veces Solicitado'
            FROM prestamos GROUP BY rut ORDER BY COUNT(*) DESC LIMIT 5
        """, conn)
    
    hoy_str = date.today().isoformat()
    
    # 🚨 BANDA DE ALERTAS CRÍTICAS UNIFICADA (RESTAURADA)
    # 1. Alertas de Hardware en Mora (Con botón para notificar)
    if not df_pr.empty:
        try:
            ...
            if not df_vencidos.empty:
                ...
                for idx, prestamo in df_vencidos.iterrows():
                    ...
                    with col_alerta_btn:
                        partes_nombre = str(prestamo['usuario']).split(" ")
                        primer_nombre = partes_nombre[0] if partes_nombre else "Usuario"
            ...
            print(f"❌ Error en Alertas Hardware Panel: {err_morosidad}")

    # 2. Alertas de la Bitácora (Tareas Atrasadas con cálculo de días)
    if not df_bit.empty and "estado_nota" in df_bit.columns:
        try:
            ...
            if not df_retrasos_pnl_bit.empty:
                st.warning(f"📝 **ALERTA BITÁCORA:** Hay **{len(df_retrasos_pnl_bit)} tareas pendientes** fuera de plazo en la escuela.")
                with st.expander("🔍 Ver detalles de tareas atrasadas", expanded=True):
        ...
        except Exception as err_bitacora_pnl:
            print(f"❌ Error en Alertas Bitácora Panel: {err_bitacora_pnl}")

    # 🔥 4. DIBUJO OBLIGATORIO DE LOS KPIs EN PANTALLA
    st.markdown("---")
    st.markdown("### 📈 Indicadores Clave de Rendimiento (KPIs)")
    
    col1, col2, col3, col4 = st.columns(4)
    
    total_equipos = len(df_eq) if (df_eq is not None and not df_eq.empty) else 0
    equipos_operativos = len(df_eq[df_eq["estado"] == "Operativo"]) if (df_eq is not None and not df_eq.empty and "estado" in df_eq.columns) else 0
    prestamos_activos = len(df_pr) if (df_pr is not None and not df_pr.empty) else 0
    notas_pendientes = len(df_bit[df_bit["estado_nota"] == "Pendiente"]) if (df_bit is not None and not df_bit.empty and "estado_nota" in df_bit.columns) else 0
    ...
    # 5. Gráficos de soporte Plotly
    st.markdown("---")
    st.markdown("### 🔍 Distribución del Inventario")
    col_g1, col_g2 = st.columns(2)
    with col_g1:
        if df_eq is not None and not df_eq.empty and 'estado' in df_eq.columns:
    ...
    with col_g2:
        if df_eq is not None and not df_eq.empty and 'tipo' in df_eq.columns:
    ...
    with col_f1:
        gasto_compras = (df_co["cantidad"] * df_co["costo_unitario"]).sum() if (df_co is not None and not df_co.empty) else 0
        st.metric("Total Invertido en Adquisiciones", f"${gasto_compras:,.0f} CLP")
    with col_f2:
        total_salas_reales = len(df_sal) if (df_sal is not None and not df_sal.empty) else 0
    ...
    with col_rk1:
        ...
        if df_rank_equipos is not None and not df_rank_equipos.empty:
            st.dataframe(df_rank_equipos, use_container_width=True, hide_index=True)
    with col_rank2:
        st.markdown("##### 👥 Top 5 Usuarios con Mayor Uso")
        if df_rank_usuarios is not None and not df_rank_usuarios.empty:
            st.dataframe(df_rank_usuarios, use_container_width=True, hide_index=True)
            
    st.stop() # 🛑 Mantiene el blindaje contra el código basura duplicado de abajo
...
# =========================================================================
# 📝 MÓDULO: BITÁCORA DE NOVEDADES (CON VENCIMIENTO Y ALERTAS DE RETRASO)
# =========================================================================
elif choice == "Bitácora de Notas":
    st.header("📝 Bitácora de Novedades Diarias del Laboratorio")
    
    # 🛡️ PARCHE AUTO-ACTUALIZADOR INTERNO: Asegura que la BD tenga las columnas necesarias
    with obtener_conexion() as conn:
        cursor = conn.cursor()
        columnas_nuevas = [
    ...
    # 🚨 BANDA DE ALERTAS CRÍTICAS DE LA BITÁCORA CON DESPACHO DE EMAIL
    hoy_str = date.today().isoformat()
    query_alertas_bit = """
    ...
    if not df_retrasos_bit.empty:
        st.error(f"⚠️ **AVISO DE RETRASO:** Se han detectado **{len(df_retrasos_bit)} tareas pendientes** que ya cumplieron su fecha límite.")
        ...
        for idx, fila_retraso in df_retrasos_bit.iterrows():
            ...
            with col_txt_b:
                st.markdown(f"🔴 **[ID {fila_retraso['id_nota']}] - Atraso {dias_atraso} días:** {fila_retraso['nota'][:50]}...")
                ...
                if st.button("✉️ Enviar Alerta", key=f"btn_send_mail_bit_{fila_retraso['id_nota']}_{idx}", disabled=(usuario_a_notificar is None), use_container_width=True):
                    with st.spinner("Enviando..."):
                        ...
                        if "BREVO_API_KEY" in st.secrets:
                            ...
                            asunto_api = f"🚨 ALERTA: Tarea Bitácora ID {fila_retraso['id_nota']} con Retraso" if dias_atraso > 0 else f"📅 Recordatorio: Tarea Bitácora ID {fila_retraso['id_nota']}"
                            estado_txt = f"<b style='color:red;'>VENCIDA por {dias_atraso} días</b>" if dias_atraso > 0 else "<b>Pendiente</b>"
                            
                            payload_api = {
                                # 🚀 RECUERDA: Si Brevo te da rechazo, cambia este correo por tu remitente verificado
                                "sender": {"name": "Sistema Laboratorio", "email": "no-reply@escuelafelipecubillos.cl"},
                                "to": [{"email": u_correo, "name": u_nombre}],
                                "subject": asunto_api,
                                "htmlContent": f"<html><body><h2>Bitácora Felipe Cubillos</h2><p>Hola {u_nombre}, Tarea ID {fila_retraso['id_nota']}: {fila_retraso['nota']} (Límite: {fila_retraso['fecha_vencimiento']} - {estado_txt})</p></body></html>"
                            }
                            ...
                            try:
                                ...
                                if res.status_code == 200 or res.status_code == 201:
                                    ...
                                    st.error(f"Brevo rechazó el envío. Código: {res.status_code} - Detalle: {res.text}")
    ...
    with t_bit1:
        with st.form("nueva_nota_form", clear_on_submit=True):
            ...
            f_vencimiento = col_add2.date_input("¿Cuándo se debe ejecutar? (Límite):", date.today())
            est_inicial = col_add3.selectbox("Estado Inicial:", ["Pendiente", "Ejecutada"])
            
            f_ej_inicial = "N/A"
            if est_inicial == "Ejecutada":
                f_ej_inicial = f_registro.isoformat()
            ...
            if st.form_submit_button("Guardar Nota en Bitácora"):
                if texto_nota.strip():
                    with obtener_conexion() as conn:
                        ...
                        conn.commit()
                    st.success("✅ Nota y planificación almacenadas exitosamente en la bitácora.")
                    st.rerun()
                else:
    ...
    with t_bit2:
        ...
        with obtener_conexion() as conn:
            df_notas_disp = pd.read_sql_query("SELECT id_nota, fecha, hora, nota, estado_nota, fecha_ejecucion, fecha_vencimiento FROM bitacora ORDER BY id_nota DESC LIMIT 50", conn)
        
        if df_notas_disp.empty:
            st.info("No hay novedades registradas para modificar.")
        else:
            opciones_notas = [f"ID:{row['id_nota']} | Est: {row['estado_nota']} | Vence: {row['fecha_vencimiento']} | {row['nota'][:30]}..." for _, row in df_notas_disp.iterrows()]
            ...
            fecha_venc_actual = str(registro_fila['fecha_vencimiento'])
            ...
            with st.form("form_modificar_nota"):
                ...
                val_venc_def = date.today() if fecha_venc_actual == "N/A" else date.fromisoformat(fecha_venc_actual)
                nuevo_vencimiento = col_mod1.date_input("Modificar fecha de ejecución límite:", val_venc_def)
                nuevo_estado = col_mod2.selectbox("Cambiar Estado de la Nota:", ["Pendiente", "Ejecutada"], index=["Pendiente", "Ejecutada"].index(estado_actual) if estado_actual in ["Pendiente", "Ejecutada"] else 0)
                ...
                if nuevo_estado == "Ejecutada":
                    ...
                    col_mod3.info("⏳ Nota marcada como Pendiente.")
        ...
        if st.session_state["rol"] == "Administrador":
            ...
            if df_notas_del.empty:
                ...
                with st.form("eliminar_nota_form"):
                    opciones_del = [f"ID:{row['id_nota']} | {row['fecha']} - {row['nota'][:40]}..." for _, row in df_notas_del.iterrows()]
                    nota_a_eliminar = st.selectbox("Selecciona la nota que deseas borrar permanentemente:", opciones_del, key="sb_del_nota")
                    
                    id_nota_del = int(nota_a_eliminar.split(" | ")[0].split(":")[1])
                ...
                with col_btn_indiv:
                    ...
                    if st.button(f"Notificar a {primer_nom_indiv}", key=llave_boton_indiv, use_container_width=True):
                        with st.spinner("Enviando correo..."):
                            ...
                            if exito_indiv_send:
                                st.toast("¡Aviso despachado!", icon="✅")
                            else:
...
# =========================================================================
    ...
    # 🚨 BANDA DE ALERTAS CRÍTICAS UNIFICADA (HARDWARE + BITÁCORA)
    # 1. Alertas de Equipos en Mora
    if not df_pr.empty:
        try:
            df_vencidos = df_pr[df_pr['fecha_limite'] < hoy_str].copy()
            if not df_vencidos.empty:
    ...
    # 2. Alertas de la Bitácora (Tareas Atrasadas)
    try:
        query_alertas_pnl_bit = """
            SELECT id_nota, nota, fecha_vencimiento FROM bitacora 
            WHERE estado_nota = 'Pendiente' AND fecha_vencimiento != 'N/A' AND fecha_vencimiento < ?
        """
        with obtener_conexion() as conn:
            df_retrasos_pnl_bit = pd.read_sql_query(query_alertas_pnl_bit, conn, params=(hoy_str,))
...
# =========================================================================
# 📊 MÓDULO: PANEL DE CONTROL Y MÉTRICAS GENERALES (VERSION UNIFICADA FIJA)
# =========================================================================
elif choice == "Panel de Control":
    st.header("📊 Resumen General y Métricas del Laboratorio")
    ...
    with obtener_conexion() as conn:
        ...
        # Consulta protectora de bitácora
        try:
            df_bit = pd.read_sql_query("SELECT estado_nota FROM bitacora", conn)
        except Exception:
            df_bit = pd.DataFrame(columns=["estado_nota"])
    ...
    # 🚀 2. RENDERIZADO INMEDIATO DE KPIs (MÉTRICAS)
    st.markdown("---")
    st.markdown("### 📈 Indicadores Clave de Rendimiento (KPIs)")
    ...
    # 5. Renderizado de Rankings de Uso
    st.markdown("---")
    st.markdown("### 🏆 Historial de Alta Demanda y Uso del Laboratorio")
    col_rk1, col_rank2 = st.columns(2)
    with col_rk1:
    ...
    if df_eq is not None and not df_eq.empty and 'ubicacion' in df_eq.columns:
