            ...
            if submitted:
                ...
                if "claves_acceso" in st.secrets and pwd_input == st.secrets["claves_acceso"].get("PASSWORD_ADMIN") and usr_input in ["admin", "Administrador", st.secrets["claves_acceso"].get("PASSWORD_ADMIN")]:
                    ...
                    st.session_state["modulos"] = [
                        ...
                        "Préstamo de Equipos", "Bitácora de Notas", "Gestión de Compras", 
                        "Gestión de Salas", "Mantenedor de Usuarios", "Mantenedor de Cuentas", 
                        "Respaldo de Seguridad", "Gestor de Informes" # <--- NUEVO MÓDULO AQUÍ
    ]
...
# =========================================================================
    ...
    # Declaración de las 5 pestañas de trabajo
        ...
        if st.session_state["rol"] == "Administrador" and lista_equipos_bd:
            ...
            if datos_act:
                with st.form("form_modificar_equipo"):
                    ...
                    if st.form_submit_button("Actualizar Ficha Técnico-Contable"):
                        with obtener_conexion() as conn:
    ...
    with t_eq3:
        if st.session_state["rol"] == "Administrador" and lista_equipos_bd:
            ...
            st.warning("⚠️ ¡Atención! Eliminar el registro removerá permanentemente el dispositivo de las auditorías contables. Si el equipo falló o está obsoleto, use la pestaña 'Dar de Baja'.")
            if st.button("🚨 Confirmar Eliminación Permanente", key="btn_del_real_eq", use_container_width=True):
                with obtener_conexion() as conn:
                    conn.cursor().execute("DELETE FROM equipos WHERE id_equipo = ?", (id_del_real,))
                    conn.commit()
                st.success("Equipo removido de la base de datos.")
                st.rerun()
        elif not lista_equipos_bd:
    ...
    with t_eq4:
        ...
        st.markdown("#### 🔍 Buscador Inteligente (Directo en Base de Datos)")
        ...
        if not df_nomina_total.empty:
            ...
            col_m2.metric("Inversión Global Sincronizada", f"${df_nomina_total['Costo ($)'].sum():,.0f} CLP")
            
            st.dataframe(df_nomina_total, use_container_width=True, hide_index=True)
            st.download_button(label="📥 Descargar Reporte Completo (Excel)", data=to_excel(df_nomina_total), file_name="inventario_general_escuela.xlsx", use_container_width=True)
        else:
            st.info("No se encontraron registros cargados con esos criterios de búsqueda.")
    # --- PESTAÑA 5: DAR DE BAJA CON ACTA AUTOMÁTICA ---
    with t_eq5:
        st.subheader("❌ Proceso de Baja Técnica Definitiva")
        st.caption("Permite retirar un activo del flujo operativo, cambiar su estado a 'De Baja' y emitir el acta correspondiente.")
        
        if st.session_state.get("rol") == "Administrador":
            if not lista_equipos_operativos:
                st.info("👍 Todos los equipos se encuentran operativos o no hay inventario disponible para descarte.")
            else:
                with st.form("form_baja_tecnica_inventario", clear_on_submit=False):
                    col_b1, col_b2 = st.columns(2)
                    eq_baja_sel = col_b1.selectbox("Selecciona el Equipo a dar de Baja:", lista_equipos_operativos, key="sb_eq_baja_master")
                    responsable_baja = col_b2.selectbox("Técnico / Evaluador que autoriza:", lista_firmas_tecnicas)
                    
                    motivo_baja = st.selectbox("Motivo principal del descarte:", [
                    ...
                    obs_baja = st.text_area("Observaciones técnicas y desglose de piezas salvadas (si aplica):", 
                                            placeholder="Ej: Se extrae memoria RAM y disco duro en buen estado para repuestos. Pantalla quebrada.")
                    
                    btn_procesar_baja = st.form_submit_button("🚨 Confirmar y Procesar Baja Técnica", use_container_width=True)
                    
                    if btn_procesar_baja:
                        if obs_baja.strip() == "":
                            st.error("⛔ Por integridad institucional, debes añadir una breve descripción técnica en las observaciones.")
                        else:
                            id_eq_real_baja = eq_baja_sel.split(" - ")[0].strip()
                            hoy_str = date.today().isoformat()
                            
                            # Transacción única: cambia estado físico y documenta en bitácora interna
                            with obtener_conexion() as conn:
                                cursor = conn.cursor()
                            ...
                            st.success(f"🚀 El equipo {id_eq_real_baja} fue dado de baja del sistema correctamente.")
                            ...
                            st.rerun()
                # --- RENDERIZADO DEL ACTA DE BAJA EMITIDA ---
                if "ultimo_acta_baja" in st.session_state:
                    st.markdown("---")
                    st.markdown("### 🖨️ Documento de Baja Técnica Generado")
                    st.info("💡 Presiona **Ctrl + P** para mandar a la impresora física o guardar directamente como PDF.")
                    ...
                    st.markdown("""
                        ...
                        @media print {
                            ...
                            form, h1, h2, h3, hr, p:not(.print-txt), span:not(.print-txt), div:not(.zona-baja) { 
                            ...
                            .zona-baja {
                                display: block !important; padding: 15mm !important; font-family: 'Arial', sans-serif !important; color: #000 !important; background: #fff !important; line-height: 1.6 !important; border: none !important;
                            }
                            * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
                            @page { margin: 10mm !important; size: letter; }
                        }
                        </style>
                    """, unsafe_allow_html=True)
                    
                    html_baja = f"""
                    <div class="zona-baja" style="padding: 25px; border: 2px solid #333; background-color: #ffffff; border-radius: 4px; color: #000;">
                        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px;">
                            {imagen_logo_baja}
                            <div style="text-align: right;">
                                <h3 class="print-txt" style="margin: 0; color: #000; font-size: 18px;">ACTA DE BAJA TÉCNICA DEFINITIVA</h3>
                                <p class="print-txt" style="margin: 3px 0 0 0; font-size: 12px; font-weight: bold; text-transform: uppercase;">{nombre_institucion}</p>
                                <p class="print-txt" style="margin: 2px 0 0 0; font-size: 11px; color: #444;">Ref: REG-BAJA-{datos_baja['id_equipo']}</p>
                            </div>
                        </div>
                        <hr style="border: 1px solid #000; margin-bottom: 20px;">
                        
                        <p class="print-txt" style="font-size: 14px; text-align: justify;">
                            En Talca, con fecha <b>{datos_baja['fecha']}</b>, la unidad de Gestión Tecnológica y Soporte de la <b>{nombre_institucion}</b> procede a la revisión, descarte y baja administrativa del inventario activo del siguiente componente tecnológico:
                        </p>
                        
                        <table class="print-txt" style="width: 100%; border-collapse: collapse; margin: 20px 0; font-size: 14px;">
                            <tr style="background-color: #f2f2f2;">
                                <th style="border: 1px solid #000; padding: 10px; text-align: left; width: 35%;">Especificación</th>
                                <th style="border: 1px solid #000; padding: 10px; text-align: left;">Detalle del Activo Fijo</th>
                            </tr>
                            <tr>
                                <td style="border: 1px solid #000; padding: 10px;"><b>Código Único ID:</b></td>
                                <td style="border: 1px solid #000; padding: 10px; font-family: monospace; font-weight: bold;">{datos_baja['id_equipo']}</td>
                            </tr>
                            <tr>
                                <td style="border: 1px solid #000; padding: 10px;"><b>Descripción / Tipo:</b></td>
                                <td style="border: 1px solid #000; padding: 10px;">{datos_baja['equipo_txt']}</td>
                            </tr>
                            <tr>
                                <td style="border: 1px solid #000; padding: 10px;"><b>Causa de la Baja:</b></td>
                                <td style="border: 1px solid #000; padding: 10px; color: #c00; font-weight: bold;">{datos_baja['motivo']}</td>
                            </tr>
                            <tr>
                                <td style="border: 1px solid #000; padding: 10px;"><b>Evaluador Técnico:</b></td>
                                <td style="border: 1px solid #000; padding: 10px;">{datos_baja['autoriza']}</td>
                            </tr>
                        </table>
                        
                        <div class="print-txt" style="margin: 20px 0; padding: 12px; border: 1px solid #000; background: #fafafa; font-size: 13px; text-align: justify;">
                            <b>Informe Técnico y Destino de Componentes:</b><br>
                            {datos_baja['observaciones']}
                        </div>
                        
                        <p class="print-txt" style="font-size: 11px; text-align: justify; color: #333; font-style: italic;">
                            Cláusula: El activo tecnológico descrito arriba deja de formar parte de la contabilidad física y operativa del colegio. Se autoriza su traslado a la Bodega de Almacenamiento de Desecho Tecnológico (RAEE).
                        </p>
                        
                        <div class="print-txt" style="margin-top: 60px; display: flex; justify-content: space-between; font-size: 13px;">
                            <div style="text-align: center; width: 45%; border-top: 1px solid #000; padding-top: 5px;">
                                <b>Firma Técnico Evaluador</b><br>Unidad Soporte TI
                            </div>
                            <div style="text-align: center; width: 45%; border-top: 1px solid #000; padding-top: 5px;">
                                <b>Firma Dirección / Validación</b><br>{nombre_institucion}
                            </div>
                        </div>
                    </div>
                    """
                    
                    st.markdown(html_baja, unsafe_allow_html=True)
                    
                    col_btn_b1, col_btn_b2 = st.columns(2)
                    with col_btn_b1:
                        html_descarga_baja = f"""<!DOCTYPE html><html><head><meta charset='utf-8'><title>Acta_Baja_{datos_baja['id_equipo']}</title></head><body style='padding:20px;'>{html_baja}</body></html>"""
                        st.download_button(label="📥 Descargar Acta (.html)", data=html_descarga_baja, file_name=f"acta_baja_{datos_baja['id_equipo']}.html", mime="text/html", use_container_width=True)
                    with col_btn_b2:
                        if st.button("🔄 Siguiente Baja / Limpiar", use_container_width=True):
                            if "ultimo_acta_baja" in st.session_state:
                                del st.session_state["ultimo_acta_baja"]
                            st.rerun()
        else:
            st.warning("🔒 Acceso Restringido: Solo un perfil Administrador puede tramitar bajas de hardware.")
