...
# Reemplaza la sección de credenciales por esta estructura:
    ...
    try:
        server = smtplib.SMTP("://office365.com", 587, timeout=10)
        server.ehlo()
        server.starttls()
        server.ehlo()
...
def ejecutar_despacho_automatico_matutino():
    """Escanea el hardware atrasado y despacha alertas por correo automáticamente una vez al día"""
        
    hoy_str = date.today().isoformat()
    
    with obtener_conexion() as conn:
        ya_enviado = conn.cursor().execute("SELECT estado FROM control_notificaciones WHERE fecha_envio = ?", (hoy_str,)).fetchone()
    
    if not ya_enviado:
        query_mora_automatica = """
            SELECT p.id_prestamo, p.id_equipo, p.usuario, p.fecha_limite, u.correo 
            FROM prestamos p 
            INNER JOIN usuarios u ON p.rut = u.rut 
            WHERE p.estado_prestamo = 'Activo' AND p.fecha_limite < ?
        """
        with obtener_conexion() as conn:
            df_vencidos_auto = pd.read_sql_query(query_mora_automatica, conn, params=(hoy_str,))
        
        if not df_vencidos_auto.empty and "SMTP_CORREO_EMISOR" in st.secrets:
            alertas_enviadas_con_exito = 0
            for _, fila in df_vencidos_auto.iterrows():
                try:
                    f_lim = date.fromisoformat(str(fila['fecha_limite']))
                    dias_atraso = max((date.today() - f_lim).days, 1)
                except Exception:
                    dias_atraso = 1
                
                if enviar_correo_mora_local(fila['correo'], fila['usuario'], fila['id_equipo'], fila['fecha_limite'], dias_atraso):
                    alertas_enviadas_con_exito += 1
            
            with obtener_conexion() as conn:
                conn.cursor().execute(
                    "INSERT INTO bitacora (nota, fecha, hora, estado_nota, fecha_ejecucion, fecha_vencimiento) VALUES (?, ?, ?, 'Ejecutada', ?, 'N/A')",
                    (f"🤖 [SISTEMA] Alerta automática matutina enviada. Se notificaron {alertas_enviadas_con_exito} préstamos atrasados.", hoy_str, datetime.now().strftime("%H:%M:%S"), hoy_str)
                )
                conn.commit()
                
        with obtener_conexion() as conn:
            try:
                conn.cursor().execute("INSERT INTO control_notificaciones VALUES (?, 'Procesado')", (hoy_str,))
                conn.commit()
            except sqlite3.IntegrityError:
                pass
                
# --- FUNCIÓN B: DESPACHO AUTOMÁTICO DE NOTAS/TAREAS CADUCADAS ---
# --- TAB 1: REGISTRAR PRÉSTAMO Y GENERAR COMODATO (ROBOT DE NOTAS AUTOMÁTICAS REVISADO) ---
def ejecutar_reporte_bitacora_vencida_automatico(correo_supervisor):
    ...
    if "SMTP_CORREO_EMISOR" not in st.secrets:
        return False
    ...
    if not ya_enviado_bit:
        ...
        if not df_bit_vencidas.empty:
            ...
            try:
                ...
                # Envío directo usando el nuevo motor SMTP de Gmail
...
CORREO_SUPERVISOR_TI = "supervisor@escuelafelipecubillos.cl"
        ...
        if not df_usuarios_check.empty and not df_libres.empty:
            ...
            with st.form("form_p_comodato", clear_on_submit=False):
                ...
                if btn_guardar:
                    ...
                    st.rerun()
            # --- TAB 1: REGISTRAR PRÉSTAMO Y GENERAR COMODATO (PARTE 3 DE 4) ---
...
# =========================================================================
    ...
    with obtener_conexion() as conn:
        ...
        for col_name, col_type in columnas_nuevas:
            try: cursor.execute(f"ALTER TABLE bitacora ADD COLUMN {col_name} {col_type}")
            ...
            with col_btn_b:
                if st.button("✉️ Enviar Alerta", key=f"btn_send_mail_bit_{fila_retraso['id_nota']}_{idx}", disabled=(usuario_a_notificar is None), use_container_width=True):
                    with st.spinner("Enviando..."):
                        ...
                        u_nombre = partes_usuario[1].strip() if len(partes_usuario) > 1 else partes_usuario[0]
                        u_correo = partes_usuario[1].strip() if len(partes_usuario) > 1 else "correo@defecto.cl"
                        
                        # CONFIGURACIÓN DIRECTA DE TU CUENTA NUEVA DE OUTLOOK
                        # Reemplaza la sección de credenciales por esta estructura:
                        remitente_b = st.secrets["email"]["sender"]
                        contrasena_b = st.secrets["email"]["password"]                        
                        try:
                            asunto_api = f"🚨 ALERTA: Tarea Demorada ID {fila_retraso['id_nota']} en el Laboratorio"
                            html_content = f"<html><body><h2>Bitácora Felipe Cubillos</h2><p>Hola {u_nombre}, se solicita la ejecución de la siguiente tarea demorada por {dias_atraso} días:<br><br><b>Tarea ID {fila_retraso['id_nota']}:</b> {fila_retraso['nota']}<br><b>Fecha Máxima Planificada:</b> {fila_retraso['fecha_vencimiento']}</p></body></html>"
                            
                            msg_b = MIMEText(html_content, 'html', 'utf-8')
                            msg_b['Subject'] = Header(asunto_api, 'utf-8')
                            msg_b['From'] = Header(f"Sistema Laboratorio <{remitente_b}>", 'utf-8')
                            msg_b['To'] = Header(u_correo, 'utf-8')
                            
                            server_b = smtplib.SMTP("://office365.com", 587, timeout=10)
                            server_b.ehlo()
                            server_b.starttls()
                            server_b.ehlo()
                            
                            server_b.login(remitente_b, contrasena_b)
                            server_b.sendmail(remitente_b, [u_correo], msg_b.as_string())
                            server_b.quit()
                            
                            st.toast(f"¡Aviso enviado a {u_nombre}!", icon="✅")
                        except Exception as e_api:
                            st.error(f"Error de conexión SMTP Outlook: {e_api}")
        st.markdown("---")
