...
def inicializar_db_tablets():
    """Asegura la creación de las estructuras lógicas dentro del archivo institucional laboratorio.db"""
    with obtener_conexion() as conn:
        ...
        """)
        conn.commit()
        # Patches de migración para el Control Clínico de Retornos
        try:
...
def renderizar_modulo_tablets(nombre_institucion):
    ...
    # Carga de datos inicial directa desde el archivo centralizado
    with obtener_conexion() as conn:
    ...
    # Creación e Instanciación de las 5 Pestañas Gráficas de Control
    ...
    # --- PESTAÑA 2: AGENDAMIENTO PEDAGÓGICO FUTURO BLINDADO (REPARACIÓN DE FORMULARIO) ---
    with tab2:
        st.subheader("📅 Agendamiento y Reserva Futura de Tablets")
        if df_usuarios.empty: st.error("❌ Registra usuarios/profesores antes de operar este módulo.")
        else:
            ...
            if df_stock_neto_futuro.empty: st.warning(f"⚠️ No hay stock disponible de tablets para el bloque {modulo_horario} en la fecha {fecha_agenda_str}.")
            else:
                ...
                # =========================================================================
                # 🚀 MEJORA CRÍTICA: SACAR AL RESPONSABLE FUERA DEL FORMULARIO PARA EVITAR EL CONGELAMIENTO
                # =========================================================================
                st.markdown("##### 👤 Selección del Personal TI Responsable")
                profesor_sel = st.selectbox("Seleccione Profesor / Custodio Responsable de la Actividad:", lista_usuarios_combo, key="sb_profesor_fuera_de_form_agenda")
                
                # Extraer el nombre de forma dinámica instantánea
                if " | " in profesor_sel:
                    profesor_nombre_sel = profesor_sel.split(" | ")[1].strip()
                else:
                    profesor_nombre_sel = profesor_sel.strip()
                
                # Verificar de inmediato si el profesor seleccionado está en mora
                esta_bloqueado = any(str(al['profesor']).strip().lower() == str(profesor_nombre_sel).strip().lower() for al in alertas_visibles)
                
                if esta_bloqueado: 
                    st.markdown(f"<div style='padding:12px; background-color:#ffcccc; color:#cc0000; border: 1px solid #cc0000; border-radius:4px; font-weight:bold; margin-bottom:15px;'>⛔ ACCESO BLOQUEADO: El docente {profesor_nombre_sel} registra deudas vigentes de tablets en mora. No se pueden agendar más equipos hasta que regularice su situación.</div>", unsafe_allow_html=True)
                
                # --- INICIO DEL FORMULARIO OPERATIVO ---
                with st.form("form_prestamo_masivo_tablets_final_agenda_nueva", clear_on_submit=False):
                    ...
                    # Botón de envío que se activa o desactiva en tiempo real según el profesor seleccionado arriba
                    if st.form_submit_button("🚀 Procesar Reserva y Emitir Acta de Entrega", disabled=esta_bloqueado):
                        ...
                        if len(tablets_candidatas) < cantidad_solicitada: st.error(f"⛔ Error: No hay suficientes unidades libres.")
                        else:
                            ...
                            with obtener_conexion() as conn:
                                ...
                                if fecha_agenda_str == date.today().isoformat():
                                    for id_t_real in lista_ids_asignados: cursor.execute("UPDATE tablets SET estado='Prestada', fecha_cambio=? WHERE id_tablet=?", (fecha_agenda_str, id_t_real))
                                conn.commit()
                                
                            modelo_txt_limpio = modelo_seleccionado_txt.split(" | ")[1].split(" (")[0].strip()
                            st.session_state["acta_tablet_entrega"] = {"id": id_generado, "tablets": ids_p_str, "modelo_txt": f"{texto_modelo_limpio} ({modelo_txt_limpio})", "cantidad": cantidad_solicitada, "usuario": nom_r, "rut": rut_r, "fecha": fecha_agenda_str, "modulo": str(modulo_horario), "curso": str(curso_clase), "asignatura": str(asignatura_clase), "obs": obs_p.strip()}
    ...
    with tab3:
        ...
        if df_prestamos_activos.empty: st.info("🤝 No hay tablets bajo préstamos activos en este momento.")
            ...
            with st.form("form_devolucion_tablets_sistema_nuevo_talca_clinico_final", clear_on_submit=False):
                ...
                if st.form_submit_button("📥 Procesar Retorno y Emitir Acta de Recepción"):
    ...
    # =========================================================================
    # 🖨️ MOTOR UNIFICADO DE VENTANAS EMERGENTES (IMPRESIÓN PRINCIPAL REPARADA)
    # =========================================================================
